...
def _bundle():
    return build_version_bundle(
        ...
        embedding_spec_path=ACTIVE_EMBEDDING_SPEC_PATH,
...
def test_preflight_fails_when_version_is_missing():
    bundle = replace(_bundle(), model_version="")
...
def test_preflight_fails_when_embedding_spec_fs_mismatch(tmp_path: Path):
    ...
    with ACTIVE_EMBEDDING_SPEC_PATH.open("r", encoding="utf-8") as f:
...
def test_preflight_fails_when_runtime_model_mismatches_embedding_spec():
    with pytest.raises(ValueError, match="Embedding spec model_version mismatch"):
        preflight_version_bundle(
            bundle=_bundle(),
            embedding_spec_path=ACTIVE_EMBEDDING_SPEC_PATH,
            policy_registry_path=POLICY_REGISTRY_PATH,
            feature_registry_path=FEATURE_REGISTRY_PATH,
            logged_fields={"customer_id", "fs_version"},
            runtime_embedding_model="text-embedding-3-large",
        )
